...
function processaction(action, post_id){
    ...
    if(action=='edit'){
        ...
        $("#p"+post_id).html('<textarea id="edit'+post_id+'" style="width: 99%" rows="4" cols="50">'+$("#p"+post_id).html().replace(/<br>/g, "\n")+'</textarea><br> <button type="button" onclick="postcomment('+post_id+')">Post</button> <button type="button" onclick="cancel('+post_id+')">Cancel</button>');
...
function postcomment(post_id){
    ...
    $.post(rootDir+"/lib/view_comment.php?id="+GET_id+"&p="+post_id+"&a="+a,{Editor:$("#edit"+post_id).val(),website:$( "input[name='website']" ).val()},function(result){
        if(result == "edit_success"){
            ...
            messageBox("<b>Comment successfully posted</b><br>Please note that this comment may require approval before it appears.", 5000);
        }
    });
    if(post_id>0){
        $("#p"+post_id).html($("#edit"+post_id).val().replace(/\n/g, "<br>"));
    }else{
        $("#edit"+post_id).val("");
    }
}
...
function buildTable(tableConfig, tableData, valueMap) {
    if(valueMap != null) {
        for (var translatable = 0; translatable < valueMap.length; translatable++) {
            var currentMap = valueMap[translatable];
            var key = Object.keys(currentMap)[0];
            for (var row = 0; row < tableData.length; row++) {
                tableData[row][key] = currentMap[key][tableData[row][key]];
            }
        }
    }
...
function deleteAttachment(fileId, formId){
    $.get( "./lib/upload.php?a=delete&file="+fileId+"&form="+formId, function( data ) {
        var selector = "#attachment_" + fileId + "_" + formId + " > .attachment";
        $(selector).html(data);
    });
}