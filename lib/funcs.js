...
function CreateWindow(winStruct){
    ...
    var titleBar = "";
    if(showtitleBar){
        titleBar = '<div style="background:'+titlecolor+'" id="titlebar" >'+winStruct.title+closeButton+'</div>';
    }
    
    var window = '<div id="win" style="border: 1px solid #428AFF; position: fixed; left: '+winStruct.x+'; top: '+winStruct.y+'; width: '+winStruct.width+'; height: '+winStruct.height+'; background: '+color+';">'+titleBar+'<div id="wincontent" style="height: 100%; overflow:'+overflow+';"></div></div>';
    var win = $(window);
    win.appendTo('body');
    win.draggable({ 
    containment: 'window', 
