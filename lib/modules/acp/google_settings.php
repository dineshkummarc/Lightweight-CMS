...
class google_settings{
    var $module_info = array(
        'title' => "logs",
        'MODULES' => array(
            array('name' => 'google_settings','permissions' => 'a_manage_board_settings'),
        )
    );
    ...
    $lng_js = array_to_js($lng,"lang",true, true);
        ...
        switch($module){
            case 'google_settings':
                ...
                $this->page_title = $language['module_titles']['google_settings'];
                $this->template = "board_settings";
                $permissions = get_table_contents("general", 'ALL'," WHERE readonly=0 AND class='google'");
